...
import {TrainingPopupPortal} from "../TrainingPopup/TrainingPopup";
import {MatchPopupPortal} from "../MatchPopup/MatchPopup";
...
export const ActionDropdownButton: FC<ActionDropdownButtonProps> = ({
  ...
  const [isOpen, setIsOpen] = useState(false)
  const [isActiveTrainingPopup, setIsActiveTrainingPopup] = useState(false)
  const [isActiveMatchPopup, setIsActiveMatchPopup] = useState(false)
  const [selectedOption, setSelectedOption] = useState(options[0])

  const openTrainingPopup = () => setIsActiveTrainingPopup(true)
  const closeTrainingPopup = () => setIsActiveTrainingPopup(false)

  const openMatchPopup = () => setIsActiveMatchPopup(true)
  const closeMatchPopup = () => setIsActiveMatchPopup(false)
  ...
  return (
    <>
      <div className={classnames(`relative ${classNames}`)}>
        ...
        {isOpen &&
          <div
            className="z-10 bg-white text-blackColor absolute inset-x-0 top-full w-full divide-y divide-gray-100 rounded-b-lg shadow">
            <ul className="text-sm">
              {options.map(option => (
                <button
                  onClick={() => onSelectOption(option)}
      ...
      {isActiveTrainingPopup && <TrainingPopupPortal onClose={closeTrainingPopup} />}
      {isActiveMatchPopup && <MatchPopupPortal onClose={closeMatchPopup}/>}
    </>
